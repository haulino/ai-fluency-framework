# Changelog

## [v1.2.1] - 2026-05-03

### Added
- Explicit coursework link description in README.md

### Changed
- Switch license from MIT to CC BY-NC-SA 4.0

## [v1.2.0] - 2026-05-03

### Added
- Discernment competency exercise
- Discernment key takeaways
- README.md reference

## [v1.1.0] - 2026-04-03

### Added
- Description competency exercise: bad prompt makeover
- Description key takeaways reference
- Restructured README with competency exercises section

## [v1.0.0] - 2026-04-03

### Added
- AI Fluency Framework overview in README
- Course deliverable and link
- Course link reference

[v1.2.1]: https://github.com/haulino/ai-fluency-framework/releases/tag/v1.2.1
[v1.2.0]: https://github.com/haulino/ai-fluency-framework/releases/tag/v1.2.0
[v1.1.0]: https://github.com/haulino/ai-fluency-framework/releases/tag/v1.1.0
[v1.0.0]: https://github.com/haulino/ai-fluency-framework/releases/tag/v1.0.0
